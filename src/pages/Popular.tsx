...
const Popular = () => {
  ...
  useEffect(() => {
    if (searchQuery.trim() === "") {
      // Sort by popularity (simulating with rating)
  ...
  const fetchAnimes = async () => {
    try {
      ...
      // Filter to only show popular anime
      const popularAnimes = data.filter(anime => anime.isPopular);
      setAnimes(popularAnimes);

      // Sort by release year (newest first) within popular anime
      const sortedAnimes = [...popularAnimes].sort((a, b) => b.releaseYear - a.releaseYear);
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="container mx-auto px-4 py-6 md:py-12">
        <div className="flex flex-col items-center mb-6 md:mb-12">
          <h2 className="text-2xl md:text-3xl font-bold mb-2">Popular Anime</h2>
          <p className="text-foreground/70 text-center max-w-2xl text-sm md:text-base">
            Top-rated anime titles from our collection, ordered by popularity
          </p>
        </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <h4 className="font-medium text-sm md:text-base mb-1 md:mb-2">Watch or Download</h4>
                    {selectedAnime.downloadLinks.length > 0 ? (
                      <EpisodeDownloadList downloadLinks={selectedAnime.downloadLinks} animeTitle={selectedAnime.title} />
                    ) : (
